...
class SignUpForm(FlaskForm):
    email = StringField(
        ...
        validators=[
            DataRequired(),
            Email(message="Esto no parece ser un correo válido"),
        ],
    )
    password = PasswordField(
        "Contraseña", validators=[DataRequired(message="Este campo es obligatorio")]
    )
    ...
    def validate_email(self, email):
        existing_user = User.query.filter_by(
            email=email.data
        ).first()  # Consulta en la base de datos
        if existing_user:
